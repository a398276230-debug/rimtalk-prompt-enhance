...
public class Dialog_AutoSlaughter : Window
{
	...
	private void DoAnimalHeader(Rect rect1, Rect rect2)
	{
		...
		GUI.color = Color.white;
		void AddCurrentAndMaxEntries(string headerKey, [Optional][DefaultParameterValue(0f)] float extraWidthFirst, [Optional][DefaultParameterValue(0f)] float extraWidthSecond)
		{
			startX = row.FinalX;
	...
	private void DoAnimalRow(Rect rect, AutoSlaughterConfig config, int index)
	{
		...
		void DrawCurrentCol(int val, [Optional][DefaultParameterValue(null)] int? limit)
		{
			Color? color2 = null;
			if (val == 0)
			{
